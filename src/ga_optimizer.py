...
class GeneticAlgorithmOptimizer:
    ...
    def calculate_fitness(self, individual):
        ...
        Aqui está o CÓDIGO CRÍTICO do Tech Challenge: Priorizando o Recall (Sensibilidade).
        """
        ...
        # Peso 70% para Recall (evitar falsos negativos no câncer de mama)
        # Peso 30% para Especificidade (evitar alarmes falsos)
        # Nota: Você pode adicionar aqui penalidades de equidade/fairness futuramente
        fitness = (0.7 * avg_recall) + (0.3 * avg_specificity)
        
        return fitness, avg_recall, avg_specificity, avg_f1

    def select_parents(self, population, fitness_scores):
        """Seleção por Torneio (Escolhe o melhor entre um subgrupo selecionado aleatoriamente)."""
        tournament_size = 3
        selected_parents = []
    ...
    def run(self):
        ...
        for generation in range(self.generations):
            ...
            for i in range(0, len(parents), 2):
                parent1 = parents[i]
                parent2 = parents[i+1] if (i+1) < len(parents) else parents[0] # Lidar com tamanho ímpar
...
if __name__ == '__main__':
    # Exemplo de como usar (Para teste interno)
    # Ler os dados já processados na Fase 1
    import os
    print("Carregando base de dados...")
    # Ajuste o caminho se necessário para rodar local
    # X_train = pd.read_csv('../data/processed/X_train_scaled.csv')
    # y_train = pd.read_csv('../data/processed/y_train.csv')
    
    # Execução simulada com df fake apenas para validar a classe
    # ga = GeneticAlgorithmOptimizer(X_train, y_train, population_size=10, generations=5)
    # best_params = ga.run()
    pass

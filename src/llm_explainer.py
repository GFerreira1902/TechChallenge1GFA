...
class MedicalLLMExplainer:
    def __init__(self, api_key=None, history_path='../outputs/llm_history.json'):
        ...
        self.client = Groq(api_key=self.api_key, timeout=20.0) # Timeout forçado de 20s
        self.model = "llama-3.3-70b-versatile" # Modelo atualizado e ativo
        
        # Histórico para a Fase 3 (Fine-tuning)
        self.history_path = history_path
        os.makedirs(os.path.dirname(self.history_path), exist_ok=True)
        
    def _save_to_history(self, prompt, response, model_prediction, metadata=None):
        """
        Salva o prompt e a resposta para criar a base de dados (Requisito da Fase 3).
        """
        entry = {
    ...
    def generate_explanation(self, prediction_class, confidence, top_features):
        ...
        # Contexto exigido pelo edital (Sensibilidade, Direcionamento prático e Linguagem não alarmista)
        system_prompt = (
        ...
        try:
            chat_completion = self.client.chat.completions.create(
                ...
                model=self.model,
                temperature=0.4, # Temperatura baixa para gerar laudos mais técnicos e consistentes
                max_tokens=800
            )
            
            resposta_llm = chat_completion.choices[0].message.content
            
            # Requisito obrigatório do FIAP: Salvar logs
